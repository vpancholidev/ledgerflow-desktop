...
//#endregion
//#region electron/database/schema.ts
...
//#endregion
//#region electron/database/index.ts
var dbPath = path$1.join(app.getPath("userData"), "ledgerflow_data.db");
...
//#endregion
//#region electron/database/handlers.ts
function registerHandlers() {
	...
	ipcMain.handle("select-files", async (event, maxFiles = 2) => {
		...
		if (toProcess.length > maxFiles) toProcess = toProcess.slice(0, maxFiles);
		const savedNames = [];
		for (const sourcePath of toProcess) try {
			const ext = path$1.extname(sourcePath);
			const newName = crypto$1.randomUUID() + ext;
			const destPath = path$1.join(uploadsDir, newName);
	...
	const getMachineId = () => {
		...
		return crypto$1.createHash("sha256").update(raw).digest("hex").substring(0, 12).toUpperCase();
	};
	const generateValidLicense = () => {
		const secretSalt = "VAIBHAV_SOFTWARE_LOCKED_2026_X99";
		const id = getMachineId();
		return crypto$1.createHash("sha256").update(id + secretSalt).digest("hex").substring(0, 16).toUpperCase().match(/.{4}/g)?.join("-") || "";
	};
	...
	ipcMain.handle("get-auth-status", async () => {
		...
		return {
			isLicensed: licenseRec[0]?.value === generateValidLicense(),
			hasPin: !!pinRec[0]?.value
		};
	});
	ipcMain.handle("activate-license", async (_, key) => {
		const expected = generateValidLicense();
		if (key.trim().toUpperCase() === expected) {
			if ((await db.select().from(appSettings).where(eq(appSettings.key, "licenseKey"))).length > 0) await db.update(appSettings).set({ value: expected }).where(eq(appSettings.key, "licenseKey"));
			else await db.insert(appSettings).values({
				key: "licenseKey",
				value: expected
			});
			return true;
	...
	ipcMain.handle("set-pin", async (_, pin) => {
		const hash = crypto$1.createHash("sha256").update(pin).digest("hex");
		if ((await db.select().from(appSettings).where(eq(appSettings.key, "dailyPin"))).length > 0) await db.update(appSettings).set({ value: hash }).where(eq(appSettings.key, "dailyPin"));
		else await db.insert(appSettings).values({
			key: "dailyPin",
			value: hash
		});
	...
	ipcMain.handle("verify-pin", async (_, pin) => {
		const hash = crypto$1.createHash("sha256").update(pin).digest("hex");
		return (await db.select().from(appSettings).where(eq(appSettings.key, "dailyPin")))[0]?.value === hash;
	});
	ipcMain.handle("reset-pin-via-license", async (_, key) => {
		const expected = generateValidLicense();
		if (key.trim().toUpperCase() === expected) {
	...
	ipcMain.handle("backup-to-cloud", async (_, { url, key }) => {
		try {
			...
			const dbPath = path$1.join(app.getPath("userData"), "ledgerflow_data.db");
			...
			if (fs.existsSync(uploadsDir)) {
				...
				for (const file of files) {
					const filePath = path$1.join(uploadsDir, file);
	...
	ipcMain.handle("save-supabase-config", async (_, { url, key }) => {
		...
		if (url) await db.insert(appSettings).values({
			key: "supabaseUrl",
			value: url
		});
		if (key) await db.insert(appSettings).values({
			key: "supabaseKey",
			value: key
		});
		return true;
